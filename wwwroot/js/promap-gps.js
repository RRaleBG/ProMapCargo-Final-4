...
window.ProMap.Gps = (() => {
    ...
    function normalizePosition(position) {
        ...
        const accuracy = Number.isFinite(accuracyValue) ? accuracyValue : null;

        const heading = Number.isFinite(headingValue) ? headingValue : null;

        const speed = Number.isFinite(speedValue) ? speedValue : null;

        const timestampValue = Number(position.timestamp);
    ...
    function start(options = {}) {
        if (!isSupported()) {
            state.enabled = false;
            state.error = new Error("Browser ne podržava GPS/geolocation.");
        ...
        watchId = navigator.geolocation.watchPosition(
            ...
            (error) => {
                 ...
                 */
                if (error && error.code === GeolocationPositionError.TIMEOUT) {
                    console.debug("[ProMap GPS] GPS timeout - čekam novi fix.");
